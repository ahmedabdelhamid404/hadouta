...
|---|---|---|
| **Customer-facing web** | Next.js 15 + React 19 + shadcn/ui + Tailwind 4 + next-intl | Cloudflare Pages |
...
- **`hadouta-web`** — Next.js 15 customer + admin app, consumes backend OpenAPI for types
...
| 17 | Tech stack: Node + Hono + Vercel AI SDK + Trigger.dev + Neon + Better-Auth + Cloudflare R2 + Next.js 15 + shadcn/ui | TypeScript-everywhere; AI SDK is React-first; Trigger.dev waitpoints save weeks; Neon avoids Supabase scaling concerns |
...
- **Two repos**: `hadouta-backend` (Node + Hono) and `hadouta-web` (Next.js 15)
